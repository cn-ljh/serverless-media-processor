...
class ErrorHandler:
    ...
    def __init__(self):
        ...
        self.ddb_table_name = os.environ.get('DDB_TABLE_NAME')
    ...
    def _send_sns_notification(self, task_id, task_type, source_key, error_message, error_details=None):
        ...
        try:
            ...
            formatted_message = f"Error processing {task_type} for file: {source_key}\n\n"
            formatted_message += f"Task ID: {task_id}\n"
            formatted_message += f"Error: {error_message}\n\n"
            
            # Include stack trace if available
            if error_details and 'traceback' in error_details:
                formatted_message += f"Stack Trace:\n{error_details['traceback']}\n\n"
            
            # Include other error details
            if error_details:
                for key, value in error_details.items():
                    if key != 'traceback':  # Skip traceback as it's already included
            ...
            if error_details:
                message_json['error_details'] = error_details
